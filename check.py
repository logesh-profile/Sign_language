...
#  Each call saves to a UNIQUE temp file
#  so pygame never locks the same file twice
# ─────────────────────────────────────────────
pygame.mixer.init()
_tts_lock = threading.Lock()   # only one speak at a time

def speak_text(text):
    def _speak():
        with _tts_lock:
            try:
                # unique temp file every call — no permission conflict
                ...
                os.remove(tmp_path)   # clean up after done
...
#  GLARE REDUCTION
# ─────────────────────────────────────────────
GLARE_BRIGHTNESS_THRESHOLD = 180
clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(4, 4))

def reduce_glare(img_crop):
    gray       = cv2.cvtColor(img_crop, cv2.COLOR_BGR2GRAY)
    brightness = gray.mean()
    if brightness <= GLARE_BRIGHTNESS_THRESHOLD:
        return img_crop, False
...
while True:
    ...
    if hands:
        ...
        imgCrop, glare_active = reduce_glare(imgCrop)
